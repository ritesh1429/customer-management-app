...
def search_customers_endpoint(name: str = Query(..., min_length=1, description="Name search query"), db: Session = Depends(get_db)):
    """Search customers by partial name (case-insensitive)"""
    return customer_service.search_customers_by_name(db=db, name=name)
...
def list_customers_endpoint(skip: int = Query(0, ge=0), limit: int = Query(100, ge=1, le=500), db: Session = Depends(get_db)):
    """List all customers with pagination"""
    return customer_service.list_customers(db=db, skip=skip, limit=limit)
